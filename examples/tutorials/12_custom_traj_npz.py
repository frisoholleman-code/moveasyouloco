...
# ==========================================
# 6. Replay using MuJoCo Native Viewer
# ==========================================
print("\n▶️ Replaying Trajectory...")
import time
import mujoco.viewer

# We extract the raw MuJoCo data object from the environment
mj_data = env.get_data() if hasattr(env, 'get_data') else env.data

# Launch the interactive viewer
with mujoco.viewer.launch_passive(model, mj_data) as viewer:
    # Loop through your trajectory infinitely so you can inspect it
    while viewer.is_running():
        for i in range(N_steps):
            # 1. Force the physics engine into your exact dataset pose
            mj_data.qpos[:] = qpos_traj[i]
            mj_data.qvel[:] = qvel_traj[i]

            # 2. Tell MuJoCo to calculate where the 3D meshes should be drawn
            mujoco.mj_forward(model, mj_data)

            # 3. Update the viewer window
            viewer.sync()

            # 4. Sleep to match the exact framerate of your OpenCap video
            time.sleep(dt)

            # Break if the user closes the window mid-loop
